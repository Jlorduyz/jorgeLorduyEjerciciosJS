...
if (edad >= 18 && altura >= 150 && vision >= 8) {
    console.log("Hola "+nombre+", estas perfectamente capacitado para conducir");
}else if(edad < 18){
    if (altura < 150) {
        if (vision < 8) {
            console.log("Joven "+nombre+" por favor vayase para su casa, y cuidado se tropieza con una piedra");
        }else{
            console.log("Joven "+nombre+", ni su edad ni su altura lo capacitan para conducir.");
        }
    }else if (vision < 8){
       console.log("Joven "+nombre+" lastima pero su edad y problemas de vision no lo capacitan para conducir."); 
    }else{
        console.log("Joven "+nombre+" su minoria de edad no lo capacita para conducir");
    }
    
}else if (altura < 150 ) {
    if (vision < 8) {
        console.log("Amigo "+nombre+" su baja estatura y problemas de vision no lo capacitan para conducir");
    }else{
        console.log("Joven "+nombre+" no cuenta con la edad ni la altura necesaria para conducir.");
    }
}else{
    console.log("Amigo "+nombre+" su problema de vision no lo capacita para conducir");
} 
...
if (nombre == sameName) {
    ...
    if (intencionDeCompra == "si") {
        ...
        if (monto >= 1000) {
            ...
            console.log("Amigo "+nombre+" no cuenta con el dinero suficiente, se retira");
        }
    }else{
        console.log("Hasta luego señor "+nombre+" se retira de la entrada por favor");
    }
}  
...
alert("A continuacion vamos a jugar un juego e adivinanzas");
alert("tienes 3 intentos para adivinar el numero del 1 al 10");
let numeroDeIntento = 1;

let numeroingresado = prompt("Ingrese un numero , intento #" + numeroDeIntento);
...
if (numeroingresado != numeroIncognita) {
  ...
  numeroDeIntento++;
  numeroingresado = prompt("Ingrese un numero , intento #" + numeroDeIntento); //Tengo error en algun else (creo, causa que el segundo intento siempre de que es menor) y no logre solucionarlo, F.
  if (numeroingresado !== numeroIncognita) {
    if (numeroingresado > numeroIncognita) {
      alert("el numero ingresado es mayor, vuelve a intentarlo");
    } else {
    ...
    numeroDeIntento++;
    numeroingresado = prompt("Ingrese un numero , intento #" + numeroDeIntento);
    if (numeroingresado != numeroIncognita) {
      if (numeroingresado > numeroIncognita) {
        alert("el numero ingresado es mayor, ya perdiste todos tus intentos");
      } else {
        alert("el numero ingresado es menor, ya perdiste todos tus intentos");
      }
    } else {
...
let jugadorUno = prompt("ingrese piedra, papel ,tijera jugador#1 (en letra minuscula por favor)")
let jugadorDos = prompt("ingrese piedra, papel ,tijera jugador#2 (en letra minuscula por favor)")

if ((jugadorUno == "piedra" && jugadorDos == "tijera") || (jugadorUno == "papel" && jugadorDos == "piedra") || (jugadorUno == "tijera" && jugadorDos == "papel") ) {
    alert("Gano el jugador #1")
}else if ((jugadorDos == "piedra" && jugadorUno == "tijera") || (jugadorDos == "papel" && jugadorUno == "piedra") || (jugadorDos == "tijera" && jugadorUno == "papel")) {
...
let confirm = prompt("nombre: "+idNombre+"  apellido: "+idApellido+"  Fecha de nacimiento: "+idAño+"/"+idMes+"/"+idDia+"  Fecha de expedicion: "+idAñoExp+"/"+idMesEpx+"  Sexo: "+idSexo+"    Los datos ingresados fueron correctos , si o no?")
